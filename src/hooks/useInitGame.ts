...
const useInitGame = () => {
  ...
  const userHashInfoEventHandler = async (_type: string, data?: IMezonAppUserHashInfo, appData?: IMezonAppUserInfo) => {
    if (data && appData?.user.id) {
      const webAppData = data.message.web_app_data;
      const isUserExisted: boolean = (await isUserExists(appData?.user.id)) || false;
      if (isUserExisted) {
  ...
  useEffect(() => {
    ...
    if (window.Mezon?.WebView) {
      ...
      window.Mezon.WebView.onEvent(MezonAppEvent.Pong, () => {
        window.Mezon.WebView?.onEvent<IMezonAppUserInfo>(MezonAppEvent.CurrentUserInfo, userInfoEventHandler);
