...
const checkUserExists = async (userId: string): Promise<ResultResponse<CheckUserExistResponse>> => {
  return (await axiosHttp.get<ResultResponse<CheckUserExistResponse>>(`/users/exist/${userId}`)).data;
};
...
const loginMezon = async (data: LoginMezonRequest): Promise<ResultResponse<LoginMezonResponse>> => {
  return (await axiosHttp.post<ResultResponse<LoginMezonResponse>>(`/auth/login-mezon`, data)).data;
};
...
const createUser = async (createUserRequest: CreateUserRequest): Promise<ResultResponse<CreateUserResponseDto>> => {
